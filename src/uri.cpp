...
namespace g6::web {
    ...
    static constexpr ctll::fixed_string uri_regex =
        R"(^(([^:/?#]+):)?(//(([^/:?#]*))(:([0-9]+))?)?([^?#]*)(\?([^#]*))?(#(.*))?)";
    static constexpr auto uri_match =
        ctre::regular_expression<typename ctre::regex_builder<uri_regex>::type, ctre::match_method, ctre::singleline>();

    uri::uri(std::string_view input) noexcept : uri_{input} {
        if (auto m = uri_match(uri_); m) {
            scheme = m.get<2>();
            host = m.get<5>();
            port = m.get<7>();
    ...
    std::optional<net::ip_endpoint> uri::endpoint() const {
    ...
    std::string uri::unescape(std::string_view input) {
        std::string output{};
        output.reserve(input.size());
        for (size_t ii = 0; ii < input.size(); ++ii) {
