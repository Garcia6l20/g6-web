...
struct header {
  ...
  void update_payload_offset() {
    ...
    if (payload_length >= 127 &&
        payload_length <= std::numeric_limits<uint16_t>::max()) {
      mask_offset = 4;
  ...
  std::tuple<size_t, size_t> calc_payload_size(size_t requested_payload_sz,
                                               size_t max_sz) {
    size_t to_send = 0;
  ...
  void serialize(std::span<std::byte> buffer) {
    ...
    buffer[0] = std::byte(fin << 7u) | std::byte(rsv1 << 6u) |
                std::byte(rsv2 << 5u) | std::byte(rsv3 << 4u) |
                (std::byte(opcode) & std::byte(0b0000'1111));
    if (payload_length < 126) {
      buffer[1] = std::byte(mask << 7u) |
                  (std::byte(0b0111'1111) & std::byte(payload_length));
    } else if (payload_length < std::numeric_limits<uint16_t>::max()) {
