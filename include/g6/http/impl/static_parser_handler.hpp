...
using c_parser_ptr =
    g6::c_unique_ptr<detail::http_parser, init_parser<type, OwnerT>>;
//
//	// clang-format off
...
template <bool is_request> class static_parser_handler {
...
public:
  using method_or_status_t =
      std::conditional_t<is_request, http::method, http::status>;

  static_parser_handler() = default;
  static_parser_handler(static_parser_handler &&other) noexcept
      : parser_{std::move(other.parser_)}, header_field_{std::move(
                                               other.header_field_)},
        url_{std::move(other.url_)}, body_{std::move(other.body_)},
  ...
  static_parser_handler &
  operator=(const static_parser_handler &) noexcept = delete;
  ...
  std::optional<size_t> content_length() const noexcept {
    ...
    if (headers_.contains("Content-Length")) {
      ...
      size_t sz = 0;
      auto [ptr, error] =
          std::from_chars(view.data(), view.data() + view.size(), sz);
  ...
  bool parse(unifex::span<T, extent> data) {
    ...
    const auto count = execute_parser(
        reinterpret_cast<const char *>(unifex::as_bytes(data).data()),
        data.size());
    if (count < data.size()) {
  ...
  bool parse(std::string_view input) {
    return parse(input.data(), input.size());
  }
...
protected:
  ...
  static inline int on_url(detail::http_parser *parser, const char *data,
                           size_t len) {
    auto &this_ = instance(parser);
  ...
  static inline int on_status(detail::http_parser *parser, const char *data,
                              size_t len) {
    auto &this_ = instance(parser);
  ...
  static inline int on_header_field(detail::http_parser *parser,
  ...
  static inline int on_header_value(detail::http_parser *parser,
                                    const char *data, size_t len) {
    auto &this_ = instance(parser);
    if (this_.state_ == status::on_header_field) {
      this_.headers_.emplace(this_.header_field_,
  ...
  static inline int on_body(detail::http_parser *parser, const char *data,
                            size_t len) {
    auto &this_ = instance(parser);
    this_.body_ =
        unifex::as_writable_bytes(unifex::span{const_cast<char *>(data), len});
  ...
  auto execute_parser(const char *data, size_t len) {
    return http_parser_execute(parser_.get(), &http_parser_settings_, data,
                               len);
  }
