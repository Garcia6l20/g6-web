...
template <typename Socket> class server_session {
public:
  struct request;
...
public:
  ...
  friend auto tag_invoke(unifex::tag_t<net::async_recv>,
                         server_session &session) {
    using namespace unifex;
    return net::async_recv(session.socket_,
                           as_writable_bytes(span{session.buffer_})) |
  ...
  friend auto tag_invoke(unifex::tag_t<net::async_send>,
                         server_session &session, http::status status,
                         std::string_view data) {
    using namespace unifex;
    session.build_header(
        status, http::headers{{"Content-Length", std::to_string(data.size())}});
    return let(net::async_send(session.socket_,
                               as_bytes(span{session.header_data_.data(),
                                             session.header_data_.size()})),
               [data, &session](size_t) {
                 return net::async_send(
                     session.socket_, as_bytes(span{data.data(), data.size()}));
               });
  }
  ...
  struct response_stream {
    ...
    friend auto tag_invoke(unifex::tag_t<net::async_send>,
                           response_stream &stream, span<T, extent> data) {
      ...
      return sequence(
                 net::async_send(
                     stream.socket_,
                     as_bytes(span{stream.size_str.data(), stream.size_str.size()})) | discard,
  ...
  friend auto tag_invoke(unifex::tag_t<net::async_send>,
    ...
    return net::async_send(session.socket_,
                           as_bytes(span{session.header_data_.data(),
                                         session.header_data_.size()})) |
           transform(
               [&session](size_t) { return response_stream{session.socket_}; });
  }
  ...
  struct request : detail::static_parser_handler<true> {
    Socket &socket_;
    session_buffer &buffer_;
    request(Socket &socket, session_buffer &buffer) noexcept
    ...
    friend auto tag_invoke(unifex::tag_t<net::async_recv>, request &request)
      ...
      if (request.has_body()) {
        ...
        return net::async_recv(request.socket_,
                               as_writable_bytes(span{request.buffer_})) |
               transform([&](size_t bytes) {
